...
class Bonus:
    def __init__(self, space, x, y):
        ...
        space.add_collision_handler(collision_types["bonus"], collision_types["ball"])
        handler.separate = self.remove_bonus

    def remove_bonus(self, arbiter, space, data):
        bonus = arbiter.shapes[1]
        platform = arbiter.shapes[0]
        space.remove(bonus, bonus.body)
        flag = choice([1, 2])
        if flag == 1:
            window.ball.count = 300
        if flag == 2:
            window.platform.positive = 100
            window.platform.negative = 100
            x = window.platform.position.x
            y = window.platform.position.y
            window.space.remove(platform, platform.body)
            window.create(x, y)
...
class Ball(pymunk.Body):
    ...
    def shoot(self):
        ...
        direction = Vec2d(choice([(50, 250), (-50, 250)]))
    ...
    def armor(self, space):
        while self.armor_count > 0:
            handler1 = space.add_collision_handler(collision_types["cullBalls"], collision_types["ball"])
            handler2 = space.add_collision_handler(collision_types["rectangle"], collision_types["ball"])
            handler3 = space.add_collision_handler(collision_types["ordinaryBalls"], collision_types["ball"])
            handler1.begin = self.armor_weakens
            handler2.begin = self.armor_weakens
            handler3.begin = self.armor_weakens
            handler1.separate = space.remove_cull_balls
            handler3.separate = self.remove_ordinary_balls

    #def remove_ordinary_balls(self, arbiter, space, data):
    #    ordinary_balls_shape = arbiter.shapes[0]
    #    space.remove(ordinary_balls_shape, ordinary_balls_shape.body)

    def armor_weakens(self):
        self.armor_count -= 1
...
class Platform(pymunk.Body):
    def __init__(self, space, x, y, positive, negative):
        ...
        shape = pymunk.Segment(self, (self.negative, 0), (self.positive, 0), 8)
...
class GameWindow(pyglet.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        self.platform = Platform(self.space, 640, 100, 50, -50)
    ...
    def create(self, x, y):
        self.platform = Platform(self.space, x, y, 100, -100)
